...
const GoogleMap = dynamic(() => import("./GoogleMap"), { ssr: false }); // GoogleMap component dynamically loaded
...
const InputForm = () => {
  ...
  const [video, setVideo] = useState<string | null>(null);a"use client";
  import dynamic from "next/dynamic";
  import { useState } from "react";
  import { useForm } from "react-hook-form";
  ...
  const InputForm = () => {
    ...
    const handleMapClick = (event: google.maps.MapMouseEvent) => {
      if (event.latLng) {
        ...
        alert(`Clicked location's latitude is ${lat}, and longitude is ${lng}.`);
      }
    };
